...
void
getfilename(const char *filename, char *name)//从完整路径名中解析出文件名称，例如：/home/test/abc.txt,解析完成后为abc.txt
{
...
SOCKET
socket_connect(const char *hostname, int port)//连接到指定的主机和端口号
{
    if (init_socket() == -1)
        return 0;
...
SOCKET
socket_create(int port)//在port指定的端口上建立server端socket
{
    if (init_socket() == -1)
        return 0;
...
SOCKET
socket_accept(SOCKET listen_st)//server端socket开始accept的函数
{
...
int
send_work(const char *hostname, int port, const char *filename)//向hostname指定的IP地址发送filename指定的文件
{
...
int
recv_work(int port)//server端socket在port指定的端口上listen，接收来自client发送的文件
{
